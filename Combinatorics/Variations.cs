        ...
        public class EnumeratorWithRepetition : IEnumerator<IList<T>>
        {
            ...
            private void ComputeCurrent()
            {
                if (myCurrentList == null)
                {
                    ...
                    foreach (var index in myListIndexes)
            ...
            public bool MoveNext()
            {
                ...
                if (myListIndexes == null)
                {
                    ...
                    for (var i = 0; i < myParent.LowerIndex; ++i)
                        myListIndexes.Add(0);
                    carry = 0;
                }
                else
                {
                    for (var i = myListIndexes.Count - 1; i >= 0 && carry > 0; --i)
                    {
        ...
        public class EnumeratorWithoutRepetition : IEnumerator<IList<T>>
        {
            ...
            private void ComputeCurrent()
            {
                if (myCurrentList == null)
                {
                    myCurrentList = new List<T>();
                    var index = 0;
                    var currentPermutation = (IList<int>)myPermutationsEnumerator.Current;
                    for (var i = 0; i < myParent.LowerIndex; ++i)
                        myCurrentList.Add(myParent.myValues[0]);
                    for (var i = 0; i < currentPermutation.Count; ++i)
                    {
                        var position = currentPermutation[i];
